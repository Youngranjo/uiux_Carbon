<!--
  Vue wrapper for Carbon's official Web Component <cds-file-uploader-button>.
  ...
  Attributes (see custom-elements.json for the full list):
      button-kind (BUTTON_KIND) — Button kind.
      size (BUTTON_SIZE) — Button size.
      accept (string) — The file types the file input should accept, separated by space.
      disabled (boolean) — `true` if this drop container should be disabled.
      multiple (boolean) — `true` if this drop container should accept more than one files at once.
      name (string) — The name of the input.
      slot (string) — The shadow DOM slot to put this drop container in.

  Usage:
    <script setup> import FileUploaderButton from './components/FileUploaderButton.vue' </script>
    <FileUploaderButton>Example</FileUploaderButton>
-->
<script setup>
...
<template>
  <cds-file-uploader-button v-bind="$attrs"><slot /></cds-file-uploader-button>
</template>
