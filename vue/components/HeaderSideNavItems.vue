<!--
  Vue wrapper for Carbon's official Web Component <cds-header-side-nav-items>.
  ...
  Attributes (see custom-elements.json for the full list):
      has-divider (boolean) — Optionally specify if container will have a bottom divider to differentiate

  Usage:
    <script setup> import HeaderSideNavItems from './components/HeaderSideNavItems.vue' </script>
    <HeaderSideNavItems>Example</HeaderSideNavItems>
-->
<script setup>
...
<template>
  <cds-header-side-nav-items v-bind="$attrs"><slot /></cds-header-side-nav-items>
</template>
