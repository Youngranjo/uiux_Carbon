<!--
  Vue wrapper for Carbon's official Web Component <cds-menu-item-selectable>.
  ...
  Attributes (see custom-elements.json for the full list):
      label (string) — Label for the menu item selectable.
      selected (boolean) — Whether the menu item is selected or not.
      renderIcon ((() => void) | undefined) — Sets the menu item's icon.
      shortcut (string)

  Usage:
    <script setup> import MenuItemSelectable from './components/MenuItemSelectable.vue' </script>
    <MenuItemSelectable>Example</MenuItemSelectable>
-->
<script setup>
...
<template>
  <cds-menu-item-selectable v-bind="$attrs"><slot /></cds-menu-item-selectable>
</template>
