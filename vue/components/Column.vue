<!--
  Vue wrapper for Carbon's official Web Component <cds-column>.
  ...
  Attributes (see custom-elements.json for the full list):
      sm (ColumnSpec | undefined) — Specify column size
      md (ColumnSpec | undefined)
      lg (ColumnSpec | undefined)
      xlg (ColumnSpec | undefined)
      max (ColumnSpec | undefined)
      span (ColumnSpecSimple | undefined)

  Usage:
    <script setup> import Column from './components/Column.vue' </script>
...
<template>
  <cds-column v-bind="$attrs"><slot /></cds-column>
</template>
